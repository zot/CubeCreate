...
struct iqm : skelmodel
{
    ...
    bool load()
    {
        ...
        if (lua::engine.RunFile(std::string(path(cfgname))) && parts.length()) // INTENSITY: execfile(cfgname, false) && parts.length()) // configured iqm, will call the iqm* commands below
        {
