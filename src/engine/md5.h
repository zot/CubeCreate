...
struct md5 : skelmodel
{
    ...
    bool load()
    {
        ...
        if(lua::engine.RunFile(std::string(path(cfgname))) && parts.length()) // INTENSITY: execfile(cfgname, false) && parts.length()) // configured md5, will call the md5* commands below
        {
